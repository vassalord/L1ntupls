...
void jetanalysis(bool newConditions, const std::string& inputFileDirectory){
  ...
  std::string outputDirectory = "hw";  //***runNumber, triggerType, version, hw/emu/both***MAKE SURE IT EXISTS
 ...
 /* TH2F *hresJet = new TH2F("hresJet","",nJetBins, jetLo, jetHi,100,-5,5);
  ...
  if (emuOn){
	...
	l1jetET4->Fill(jetEt_4);
      ...
      for (int a = 0; a < l1CTemu_->nTower; a++){
        if (l1CTemu_->iqual[a]  == 1){
        int ietacalo = l1CTemu_->ieta[a];
        int iphicalo = l1CTemu_->iphi[a];
         if (abs(ietacalo)<16){
         std::cout  << "Event " << jentry << " : RAW calo tower: ieta, iphi " << ietacalo << " ," << iphicalo << std::endl;
                  }
                 }
             }
             
    for (int i = 0; i < l1TPemu_->nHCALTP; i++){
        if ( (l1TPemu_->hcalTPfineGrain0[i] || (!l1TPemu_->hcalTPfineGrain1[i] && (l1TPemu_->hcalTPfineGrain2[i] || l1TPemu_->hcalTPfineGrain3[i]))) == 1 ){
        int eieta = l1TPemu_->hcalTPieta[i];
        int adj_iphi = 72 - l1TPemu_->hcalTPiphi[i] + 18;
    	if (adj_iphi > 72) adj_iphi -= 72;
	if (abs(eieta)<16){
         std::cout << "Event " << jentry << " : RAW flagged HCAL TP:  ieta, iphi " << eieta << ", " << adj_iphi << std::endl;
                          }
                        }
                    }
    ...
    treeL1TPhw->GetEntry(jentry); // CaloTP
      ...
      int iphi = -999;
      //double et = 0;
      for (int i = 0; i < l1hw_->nJets; i++){
      if (l1hw_->jetHwQual[i] == 1){
	    ieta = l1hw_->jetIEta[i];
    	    iphi = (l1hw_->jetIPhi[i]+1)/2;
    	    
        std::cout << "nJets "<< l1hw_->nJets << " i "<< i << " ieta " << ieta << " iphi " << iphi << " entry " << jentry << std::endl;
    	std::ofstream out ("nJets_iphi_ieta.txt", std::ios::app);
    	if (out.is_open()){
    	 out << ieta<< " " << iphi << std :: endl;
     }
    	 out.close();
        }
        }*/
        ...
        /*for (int a = -4; a <= 4; a++){
	et += TPhw_->hcalTPet[ieta+a];
	et += TPhw_->hcalTPet[iphi+a];
	}
	std::cout << "nJets "<< l1hw_->nJets << " i "<< i << " ieta " << ieta << " iphi " << iphi << " energy " << et << " entry " << jentry << std::endl;
    
    }
    }*/
  
  
  for (int a = 0; a < l1CThw_->nTower; a++){
          if ( l1CThw_->iqual[a] == 1){
             int ietacalo = l1CThw_->ieta[a];
             int iphicalo = l1CThw_->iphi[a];
             if (abs(ietacalo)<16){
          std::cout  << "Event " << jentry << " : EMU calo tower ieta, iphi " << ietacalo << " ," << iphicalo << std::endl;
                             }
                             }
                              }
  ...
  for (int i = 0; i < l1TPhw_->nHCALTP; i++){
    if ( (l1TPhw_->hcalTPfineGrain0[i] || (!l1TPhw_->hcalTPfineGrain1[i] && (l1TPhw_->hcalTPfineGrain2[i] || l1TPhw_->hcalTPfineGrain3[i]))) == 1 ){
	int eieta = l1TPhw_->hcalTPieta[i];
	int adj_iphi = 72 - l1TPhw_->hcalTPiphi[i] + 18;
	if (adj_iphi > 72) adj_iphi -= 72;
	if (abs(eieta)<16){
  std::cout << "Event " << jentry << " : EMU flagged HCAL TP:  ieta, iphi " << eieta << ", " << adj_iphi << std::endl;
     }
     }    
      }
       }
	...
	/*if (jetIdx>=0) { // at least 1 offline jet >10. geV
	  ...
	  if (l1jetIdx>=0) { // found matched l1jet
	    ...
	    if (l1hw_->jetEt[l1jetIdx]>50.) jetET50->Fill(jet_->etCorr[jetIdx]);
	    if (l1hw_->jetEt[l1jetIdx]>64.) jetET64->Fill(jet_->etCorr[jetIdx]);
	    if (l1hw_->jetEt[l1jetIdx]>76.) jetET76->Fill(jet_->etCorr[jetIdx]);
	    if (l1hw_->jetEt[l1jetIdx]>92.) jetET92->Fill(jet_->etCorr[jetIdx]);
	    if (l1hw_->jetEt[l1jetIdx]>112.) jetET112->Fill(jet_->etCorr[jetIdx]);
	    if (l1hw_->jetEt[l1jetIdx]>180.) jetET180->Fill(jet_->etCorr[jetIdx]);

	    float resJet=(l1hw_->jetEt[l1jetIdx]-jet_->etCorr[jetIdx])/jet_->etCorr[jetIdx];
	    hresJet->Fill(jet_->etCorr[jetIdx],resJet);

	    if (fabs(l1hw_->jetEta[l1jetIdx])<=1.305) {
	      hresJet_hb->Fill(jet_->etCorr[jetIdx],resJet);
	    } else if (fabs(l1hw_->jetEta[l1jetIdx])<=3.0) {
